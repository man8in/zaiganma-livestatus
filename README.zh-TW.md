...
> [!NOTE]
> **ZaiGanMa (LiveStatus)** 是一款輕量級 MCDR 插件，允許玩家設定自己的狀態標籤，並顯示在聊天框和 TAB 列表中。基於 Minecraft 原版 Team 機制。
...
| `!!zgm lib reset` | 重設狀態庫為預設（僅限管理員） |
| `!!zgm suggest` | 隨機推薦狀態 |
| `!!zgm config` | 查看設定面板（僅限管理員） |

> [!TIP]
> 在 `!!zgm lib`、`!!zgm clib`、`!!zgm suggest`、`!!zgm config` 中點擊狀態可自動填入指令，按 Enter 確認即可。
